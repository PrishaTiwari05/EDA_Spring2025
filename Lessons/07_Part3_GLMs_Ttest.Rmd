...
```{r}
summary(EPAair$AQI_Ozone) #mean = 40.88 and median = 40
length(EPAair$AQI_Ozone) #8976 observations

O3.onesample <- t.test(EPAair$AQI_Ozone, mu = 50, alternative = "less")
O3.onesample

Ozone.plot <- ggplot(EPAair, aes(x = AQI_Ozone)) +
...
# Format as a GLM
O3.onesample2 <- lm(AQI_Ozone ~ 1, EPAair)
O3.onesample2
```
...
ggplot(EPAair, aes(x = AQI_Ozone, color = as.factor(Year))) +
  geom_freqpoly()
...
O3.twosample <- t.test(EPAair$AQI_Ozone ~ EPAair$Year)
O3.twosample

# Format as a GLM
O3.twosample2 <- lm(EPAair$AQI_Ozone ~ EPAair$Year)
