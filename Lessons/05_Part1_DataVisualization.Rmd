...
# Scatterplot
ggplot(EPAair, aes(x = Date, y = AQI_Ozone)) + 
  geom_point()

O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = AQI_Ozone))
print(O3plot)
...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = AQI_PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# Separate plot with facets
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = AQI_PM2.5, shape = as.factor(Year))) +
  geom_point() +
