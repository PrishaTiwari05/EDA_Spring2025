...
#Exercise 1:
O3density <- 
  ggplot(EPAair, aes(x = AQI_Ozone, fill = as.factor(Year))) +
...
O3box <- 
  ggplot(EPAair, aes(y = AQI_Ozone, x = as.factor(Year))) +
  geom_boxplot() +
...
pmbyozone <-
  lm(data = EPAair, AQI_PM2.5 ~ AQI_Ozone)
summary(pmbyozone)
...
ggplot(EPAair, aes(x = AQI_Ozone, y = AQI_PM2.5)) +
...
airstats <- 
  EPAair %>%
  select(meanLat, meanLong, AQI_PM2.5, AQI_Ozone) %>%
  na.omit()
airstatscorrr <- cor(airstats)
corrplot(airstatscorrr, method = "ellipse")
corrplot.mixed(airstatscorrr, upper = "ellipse")
...
pmbyall <-
  lm(data = EPAair,
     AQI_PM2.5 ~ meanLat + meanLong + Month + Year + AQI_Ozone)
step(pmbyall)
