...
summary_PM25 <- epa_data %>% 
  drop_na(AQI_PM2.5) %>% 
  group_by(COUNTY) %>% 
  summarize(mean_PM2.5 = mean(AQI_PM2.5)) %>% 
  arrange(mean_PM2.5)
...
```{r plot one, echo=FALSE, warning=FALSE, fig.align='left', fig.height= 3, fig.cap= "Monthly Ozone Levels in 2018"}

epa_2018 <- epa_data %>%
  filter(Year == 2018)

O3_2018 <- 
  ggplot(epa_2018, aes(x = Month, y = AQI_Ozone)) +
  geom_boxplot(fill = "lightblue", color = "black") +
  labs(x = "Month",
       y = "Concentration (µg/m3)",
       title = "Monthly Ozone Levels (2018)") +
  theme_minimal()

print(O3_2018)
```

```{r plot two, echo=FALSE, warning=FALSE, fig.align='left', fig.height= 3, fig.cap= "Monthly Ozone Levels in 2019"}

epa_2019 <- epa_data %>%
  filter(Year == 2019)

O3_2019 <- 
  ggplot(epa_2019, aes(x = Month, y = AQI_PM2.5)) +
  geom_boxplot(fill = "lightblue", color = "black") +
  labs(x = "Month",
       y = "Concentration (µg/m3)",
       title = "Monthly Ozone Levels (2019)") +
  theme_minimal()
...
## Git Repository
https://github.com/PrishaTiwari05/TSA_Sp25
